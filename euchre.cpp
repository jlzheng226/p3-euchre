...
class Game {
    ...
    private:
        ...
        void play_hand() {
            ...
            int highest_player_index = lead_player_num;

            for (int trick = 0; trick < 5; trick++) {
                Card leadCard = leadPlayer->lead_card(trump);
                Card highestCard = leadCard;
                int player_index;

                // print the lead card and lead player
                cout << leadCard << " led by " << leadPlayer->get_name() << endl;
                for (int i = lead_player_num + 1; i <= lead_player_num + 3; i++) {
                    player_index = i % 4;
                    Card playCard = players[player_index]->play_card(leadCard, trump);
                    // print each trick
                    ...
                    if (Card_less(highestCard, playCard, leadCard, trump)) {
                        ...
                        highest_player_index = player_index;
                    }
                }
                ...
                if (highest_player_index == 0 || highest_player_index == 2) {
                    team1.addTricksWon();
                }
                else {
                    team2.addTricksWon();
