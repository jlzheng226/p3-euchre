...
private:
   ...
   void play_hand() {
       ...
       int biggest_index = (dealer_index + 1) % 4;
       Card lead_card;
       Card biggest_card;
       Card follow_card;

       for (int trick = 1; trick < 6; trick++) {
           lead_card = (*euchre_players[biggest_index]).lead_card(trump);
           biggest_card = lead_card;
           cout << lead_card << " led by " (*euchre_players[biggest_index]).get_name() << endl;
           for (int player = 1; player < 4; player++) {
               follow_card = (*euchre_players[(biggest_index + player) % 4]).play_card(lead_card, trump);
               cout << follow_card << " played by " << (*euchre_players[(biggest_index + player) % 4]).get_name() << endl;
               if (Card_less(biggest_card, follow_card, trump)) {
                   biggest_card = follow_card;
                   biggest_index = (biggest_index + player) % 4;
               }
           }
       ...
       for (auto trick : tricks_per_player) {
           trick = 0;
       }
};
...
int main(int argc, char** argv) {
    ...
    while ((newGame.get_points_team1 < winning_points) &&
        (newGame.get_points_team2 < winning_points)) {
        newGame.play();
    }

    if (newGame.get_points_team1 >= winning_points) {
        cout << (*euchre_players[0]).get_name() << " and " << (*euchre_players[2]).get_name()
            << " win!"
    }
    else {
